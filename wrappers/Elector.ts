...
export class Elector implements Contract {
    ...
    async getParticipatesIn(provider: ContractProvider, pkey: Buffer) {
        const { stack } = await provider.get('participates_in', [{type: 'int', value: buff2bigint(pkey)}]);
        return stack.readBigNumber();
    }
